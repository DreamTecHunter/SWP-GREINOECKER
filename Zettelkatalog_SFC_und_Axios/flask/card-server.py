...
class CatalogCardREST(Resource):
    def get(self, id):
        info = CatalogCard.query.get(id)
        return jsonify(info)
    ...
    def cat_search(q):
        infos = CatalogCard.query.filter(CatalogCard.description.contains(q)).all()
        return jsonify(infos)


api.add_resource(CatalogCardREST, '/cat-item/<int:id>')
